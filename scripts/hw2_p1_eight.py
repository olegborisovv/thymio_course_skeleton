...
if __name__ == '__main__':
    ...
    start_pose = thymio.current_pose


    def travel_arc(break_condition, velocity, angle):

        for i in range(400):
            thymio.cmd_vel(velocity, angle)
            p = thymio.current_pose
            distance = thymio.euclidean_distance(start_pose)
            if break_condition(distance):
                break
            if i % 30 == 0:
    ...
    velocity = 0.15
    angle = 0.6
    travel_arc(lambda d: d > 0.2, velocity, angle)
    travel_arc(lambda d: d < 0.1, velocity, angle)
    travel_arc(lambda d: d > 0.2, velocity, -angle)
    travel_arc(lambda d: d < 0.1, velocity, -angle)
